...
FROM
    names;
-- no. of rows in names: 25,735

SELECT 
    COUNT(*)
FROM
    ratings;
-- no. of rows in ratings: 7,997

SELECT 
    COUNT(*)
FROM
    role_mapping;
-- no. of rows in role_mapping: 15,615


-- Q2. Which columns in the movie table have null values?
-- Type your code below:

-- null values can be calculated using case statements
...
-- country, worlwide_gross_income, languages, production_company have 20, 3724, 194, 528 null values respectively
...
-- Drama genre had highest movies produced overall
...
-- 3289 movies that belong to only 1 genre 
...
-- Dream Warrior Pictures and National Theatre Live have produced most number of hit movies 
...
-- No null value in Name column. Height, DOB, Known_for_movies have 17335, 13431 and 15226 null values respectively
...
AS
  ...
  -- James Mangold 4, Joe Russo	3, Anthony Russo 3 are the top 3 directors in the top three genres whose movies have an average rating > 8
...
-- Mammootty and Mohanlal are top 2 actors with 8 and 5 movies respectively
...
WITH top_3_prod_house
     AS (SELECT production_company,
         ...
         GROUP  BY production_company)
SELECT *
FROM   top_3_prod_house
WHERE  prod_comp_rank <= 3;

-- Marvel Studios, Twentieth Century Fox and Warner Bros. are the top three production houses based on the number of votes received by their movies- 	2656967, 2411163, and 2396057	respectively
...
-- Top actor is Vijay Sethupathi, then Fahadh Faasil and Yogi Babu
...
SELECT genre,
       ...
       Avg(Round(Avg(duration), 2))
         over (
           ORDER BY genre ROWS BETWEEN 10 preceding AND CURRENT ROW) AS
       moving_avg_duration
FROM   movie m
       inner join genre g
               ON m.id = g.movie_id
...
-- Star Cinema, Twentieth Century Fox are the top production houses
